...
export default {
  ...
  methods: {
    load() {
      ...
      if (location.hash) {
        fetch(
          this.$root.api.lists + location.hash.substr(1) + '/' + '?expand=subevent',
          {
      ...
      fetch(
        this.$root.api.lists +
          '?exclude=checkin_count&exclude=position_count&expand=subevent&ends_after=' +
          cutoff,
        {
          credentials: 'same-origin',
        }
